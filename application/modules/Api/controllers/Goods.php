...
class GoodsController extends ApiController
{
    ...
    function detailAction()
    {
        ...
        $data = array(
            ...
            'itempic' => $val['itempic'] . ConfigModel::IMG,
    ...
    function convertAction(){
        ...
        if(empty($itemid)) {
            if (preg_match('#\x{ffe5}([a-zA-Z0-9]{11})\x{ffe5}#isu', $content, $m)) {//淘口令
                ...
                //$itemid = $yuque_model->tpwdConvert(['password_content' => $m[0]]);
                if(empty($itemid)){
        ...
        if($tb_info) {
            ...
            $item_info = $taobao_model->TbkDgMaterialOptionalRequest(['q'=>'https://item.taobao.com/item.htm?id='.$itemid]);
            if(empty($item_info)){
                $this->responseJson('20189', '抱歉!识别不到该商品优惠信息');
            }
            $data = $taobao_model->makeMaterialTb($item_info);
    ...
    function topListAction(){
        ...
        foreach($result['result'] as &$item){
            $item = [
                ...
                'itempic' => $item['Pic'] . ConfigModel::IMG,
